...
public:
    ...
    std::function<std::vector<ReturnV>(const void*)> get_function(const std::string& key) const
    {
        ...
        if (it == data.end()) throw std::logic_error(std::string("doesn't have a serialization function for key: ") + key);
        return it->second;
    }
    ...
    void add_function(
        std::string key,
        const ISerializationFunctionsMapper<ReturnV, std::type_info>& mapper,
        FG fields_getter
        )
    {
        if (data.contains(key)) {
            throw std::logic_error(std::string("type of key: ") + key + std::string(" already has serialization function"));
        }
